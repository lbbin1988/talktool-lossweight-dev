...
"""
    ...
    def _get_backup_alternatives(self, tag: str, emotion: str, reply_text: str = "", user_input: str = "") -> List[dict]:
        """获取两条不同的备用备选话术（确保与推荐话术不同，并基于用户输入动态生成）"""
        alternatives = []
        used_texts = {reply_text} if reply_text else set()  # 记录已使用的话术
        
        # 根据用户输入动态生成备选话术
        if user_input:
            alt1_en, alt1_zh = self._generate_dynamic_response(user_input, variant=1)
            alt2_en, alt2_zh = self._generate_dynamic_response(user_input, variant=2)
            
            if alt1_en and alt1_en not in used_texts and not self._is_similar(alt1_en, reply_text):
                alternatives.append({'text': alt1_en, 'text_zh': alt1_zh})
                used_texts.add(alt1_en)
            
            if alt2_en and alt2_en not in used_texts and not self._is_similar(alt2_en, reply_text):
                alternatives.append({'text': alt2_en, 'text_zh': alt2_zh})
                used_texts.add(alt2_en)
        
        # 如果动态生成不够，使用传统方法补充
        if len(alternatives) < 2:
            templates = BACKUP_TEMPLATES.get(tag, BACKUP_TEMPLATES.get('B', {}))  # 默认使用B类（健康问题）而不是A类
            emotion_templates = templates.get(emotion, templates.get('default', []))
        ...
        while len(alternatives) < 2:
            base_text = emotion_templates[0][0] if emotion_templates else "Try my easy method!"
            alt_en = self._paraphrase_text(base_text)
            alt_zh = self._simple_translate(alt_en) if not emotion_templates else self._paraphrase_text(emotion_templates[0][1])
    ...
    def _simple_translate(self, text: str) -> str:
        ...
        # 如果中文字符太少，也用通用回复
        chinese_char_count = sum(1 for c in result if '\u4e00' <= c <= '\u9fff')
    ...
    def intercept(self, post_content: str = "", comment_content: str = "") -> BotResponse:
        ...
        # 生成备选话术（两条）- 确保与推荐话术不同
        response.alternatives = self._get_backup_alternatives(response.tag, response.emotion, response.reply_text, user_input)
